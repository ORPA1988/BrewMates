...
| B-3 | **`online_service.dart` entflechten** — *begonnen 15.08.2026*: Antwort-Typen nach `online/models.dart` (2.009 → 1.803 Z.), per `export` weitergereicht. **Rest bewusst verschoben, siehe rechts.** | M | Jede Änderung lädt heute die ganze Datei in den Kontext. **Warum nicht weiter:** Ein Split über `part` + `extension` wäre mechanisch einfach, macht die Methoden aber **statisch gebunden** — `FakeOnlineService` könnte sie nicht mehr überschreiben, und der Aufruf ginge still an der Attrappe vorbei. Der saubere Weg sind Unterobjekte (`online.sessions.end(...)`), das ändert jede Aufrufstelle. Bei **8,4 % Abdeckung** auf dieser Datei ist das genau die Ausgangslage, die am 15.08. zwei stille Regressionen erzeugt hat. **Erst Abdeckung, dann Split.** |
