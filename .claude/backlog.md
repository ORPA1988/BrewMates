...
| ~~A-9~~ | ~~**Architektur-Test hatte einen blinden Fleck**~~ - erledigt 15.08.2026: Er prueft jetzt auch relative Importe. Zwei echte Cross-Feature-Importe (map -> venues, scan -> beers) standen unbemerkt im Code, waehrend der Test gruen leuchtete. | - | `openNow` nach `data/venue_open.dart`, `StorySection` nach `widgets/`. **Ein Waechter mit blindem Fleck ist schlimmer als keiner** - er erzeugt Vertrauen, das er nicht deckt. Gefunden beim Schreiben der B-5-Tests, nicht vom Test selbst. |
...
| ~~B-2~~ | ~~**Cloud-Wiederherstellung inkrementell**~~ — erledigt 15.08.2026: erst ID-Abgleich (eine UUID je Zeile), dann nur die Lücke mit vollen Spalten. Bei „nichts Neues" entfällt der teure Abruf ganz. | — | Bewusst **kein Zeitstempel-Wasserzeichen**: `created_at` stammt vom Client, ein offline entstandener und Tage später hochgeladener Check-in läge davor und würde nie wiederhergestellt. |
| ~~B-3~~ | ~~**`online_service.dart` entflechten**~~ — erledigt 15.08.2026: Modelle nach `online/models.dart`, vier Bereiche nach `online/api/` (`friends`, `sessions`, `checkins`, `venues`). **2.009 → 819 Zeilen**, größte neue Datei 447. | — | Als **Klassen**, nicht als Extensions: Nur so kann `FakeOnlineService` sie überschreiben. Der Umbau erzeugte 56 Compilerfehler an Aufrufstellen — alle sichtbar, keiner still. Rest (`auth`, `crews`, `beers`, `challenges`) blieb im Kern; 819 Zeilen sind handhabbar. |
| ~~B-4~~ | ~~**`providers.dart` je Feature aufteilen**~~ — erledigt 15.08.2026: `data/providers/` mit acht Themendateien. **1.271 → 505 Zeilen**, größte Datei 248. | — | Hier `part` statt eigener Bibliotheken — die Provider teilen private Helfer, und es sind Variablen, keine Methoden: kein Dispatch-Problem wie bei B-3. Null Aufrufstellen geändert. |
| ~~B-5~~ | ~~**Widget-Tests für die Kernbildschirme**~~ — erledigt 15.08.2026: `friends_screen`, **Karte** und **Scan** (Eingabeweg). Abdeckung 29,9 % → 32,8 %. | — | Meine Zwischeneinschätzung „`flutter_map` läuft im Widget-Test nicht" war **falsch**: Es fehlte nur ein Kachel-Stub (ohne Netz scheitert jede Kachel und flutter_map wiederholt endlos) und sauberer Abbau der Drift-Timer. Beim Scanner braucht wirklich nur die Kamera ein Gerät; die Eingabeprüfung ist über `debugDefaultTargetPlatformOverride` erreichbar. **Beim Schreiben dieser Tests fielen zwei echte Cross-Feature-Importe auf**, die der Architektur-Test übersah — siehe A-9. |
| ~~B-6~~ | ~~**Check-ins bearbeiten**~~ — erledigt 15.08.2026: Funktion 27, Drift v13 (`checkins.dirty`), Bearbeiten-Blatt an der Check-in-Karte. | — | Keine Server-Migration nötig — `checkins_update` erlaubt es seit 0001. Der Kern war der **Abgleich**: Er lud nur hoch, was der Server noch nicht kannte; eine Korrektur wäre nie angekommen. |
