...
| A-7 | **`domain/badges.dart` und `domain/challenges.dart` von der Datenbank lösen** — sie importieren Drift nicht nur, sie fragen ab (`db.myCheckinsDetailed(...)`). Laden nach `data/`, Bewertung bleibt im Domain. | M | Beim Aufräumen von A-5 durch den neuen Architektur-Test gefunden — der Review hatte nur `statistics.dart` gesehen. Solange sie offen sind, stehen sie als Ausnahme in `test/architecture_test.dart`; **diese Liste darf nur schrumpfen**. |
