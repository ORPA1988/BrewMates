...
Die Community-Datenbank umfasst acht Dateien: `beers-at.json` und `breweries-at.json` (Österreich, 447 Biere / 46 Brauereien), `beers-by.json` und `breweries-by.json` (Bayern, 73 Biere / 33 Brauereien), `beers-de.json` und `breweries-de.json` (Deutschland ohne Bayern, 95 Biere / 40 Brauereien) sowie `beers-ch.json` und `breweries-ch.json` (Schweiz, 45 Biere / 18 Brauereien) — zusammen 660 Biere und 137 Brauereien. Biere und Brauereien sind über das Feld `brewery_id` miteinander verknüpft.

Die Bier- und Brauereidaten stammen aus dem Trainingswissen eines KI-Modells (Stand ca. 2025) und wurden redaktionell zusammengestellt — ohne Gewähr auf Vollständigkeit oder Richtigkeit einzelner Angaben (z. B. Alkoholgehalt oder Sortimentsstand). Das Feld `community_rating` ist eine konservative redaktionelle Schätzung auf Basis des allgemeinen Rufs der Biere und kein gemessener oder von einer Plattform übernommener Wert. **Seit 0.10.15 wird es in der App nicht mehr angezeigt** (Meldung #143): Als Sternebild behauptete es eine Messung, und die Werte lagen so eng beieinander, dass sie ohnehin nicht zu unterscheiden waren. Was ein Bier beschreibt, ist `description_community` — ein geschriebener Satz über Geschmack, Aroma und Ruf. Sterne zeigt die App nur für echte Bewertungen aus Check-ins. Beschreibungstexte sind Paraphrasen und keine wörtlichen Zitate der Brauereien. Korrekturen und Ergänzungen sind ausdrücklich willkommen — bitte per GitHub-Issue melden.
